...
cmd = "kubectl config set-credentials " + values_json["services"]["issuer_url"] + " --auth-provider=oidc --auth-provider-arg=client-secret= --auth-provider-arg=idp-issuer-url=" + values_json["services"]["issuer_url"] + " --auth-provider-arg=client-id=kubernetes --auth-provider-arg=refresh-token=  --auth-provider-arg=id-token=" + signed_jwt.decode("utf-8") + "  --auth-provider-arg=idp-certificate-authority-data="
system(cmd)
